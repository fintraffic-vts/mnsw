# config.json

- Source JSON: [config.json](../../schemas/config.json)
- Purpose: element-level metadata used for labels, descriptions, occurrence, status, and Finland scope filtering.

## Formality summary

| Formality | Details | All | Narrow | Required | Optional |
| --- | --- | --- | --- | --- | --- |
| ATA | [Section](#formality-ata) | 12 | 3 | 6 | 5 |
| ATD | [Section](#formality-atd) | 11 | 2 | 6 | 4 |
| CGA | [Section](#formality-cga) | 180 | 19 | 31 | 148 |
| CGD | [Section](#formality-cgd) | 148 | 21 | 31 | 116 |
| COA | [Section](#formality-coa) | 3 | 0 | 1 | 1 |
| CRT | [Section](#formality-crt) | 31 | 12 | 7 | 23 |
| CWA | [Section](#formality-cwa) | 50 | 16 | 27 | 22 |
| CWD | [Section](#formality-cwd) | 50 | 17 | 27 | 22 |
| DUE | [Section](#formality-due) | 35 | 15 | 7 | 27 |
| FRM | [Section](#formality-frm) | 0 | 0 | 0 | 0 |
| HZA | [Section](#formality-hza) | 177 | 41 | 47 | 129 |
| HZD | [Section](#formality-hzd) | 148 | 39 | 42 | 105 |
| MDD | [Section](#formality-mdd) | 31 | 20 | 13 | 17 |
| MDH | [Section](#formality-mdh) | 47 | 25 | 34 | 12 |
| MTS | [Section](#formality-mts) | 41 | 16 | 13 | 27 |
| NOA | [Section](#formality-noa) | 219 | 58 | 52 | 166 |
| NOD | [Section](#formality-nod) | 138 | 36 | 35 | 102 |
| PBK | [Section](#formality-pbk) | 0 | 0 | 0 | 0 |
| PXA | [Section](#formality-pxa) | 72 | 22 | 29 | 42 |
| PXD | [Section](#formality-pxd) | 67 | 23 | 25 | 41 |
| SEC | [Section](#formality-sec) | 73 | 40 | 40 | 32 |
| SHP | [Section](#formality-shp) | 124 | 9 | 9 | 114 |
| SID | [Section](#formality-sid) | 12 | 6 | 3 | 8 |
| SRV | [Section](#formality-srv) | 145 | 34 | 25 | 119 |
| VID | [Section](#formality-vid) | 18 | 9 | 8 | 9 |
| WAR | [Section](#formality-war) | 23 | 10 | 17 | 5 |
| WAS | [Section](#formality-was) | 51 | 11 | 26 | 24 |

<a id="formality-ata"></a>
## ATA

- All elements: 12
- Narrow (Suppeat): 3
- Required: 6
- Optional: 5

<a id="formality-atd"></a>
## ATD

- All elements: 11
- Narrow (Suppeat): 2
- Required: 6
- Optional: 4

<a id="formality-cga"></a>
## CGA

- All elements: 180
- Narrow (Suppeat): 19
- Required: 31
- Optional: 148

<a id="formality-cgd"></a>
## CGD

- All elements: 148
- Narrow (Suppeat): 21
- Required: 31
- Optional: 116

<a id="formality-coa"></a>
## COA

- All elements: 3
- Narrow (Suppeat): 0
- Required: 1
- Optional: 1

<a id="formality-crt"></a>
## CRT

- All elements: 31
- Narrow (Suppeat): 12
- Required: 7
- Optional: 23

<a id="formality-cwa"></a>
## CWA

- All elements: 50
- Narrow (Suppeat): 16
- Required: 27
- Optional: 22

<a id="formality-cwd"></a>
## CWD

- All elements: 50
- Narrow (Suppeat): 17
- Required: 27
- Optional: 22

<a id="formality-due"></a>
## DUE

- All elements: 35
- Narrow (Suppeat): 15
- Required: 7
- Optional: 27

<a id="formality-frm"></a>
## FRM

- All elements: 0
- Narrow (Suppeat): 0
- Required: 0
- Optional: 0

<a id="formality-hza"></a>
## HZA

- All elements: 177
- Narrow (Suppeat): 41
- Required: 47
- Optional: 129

<a id="formality-hzd"></a>
## HZD

- All elements: 148
- Narrow (Suppeat): 39
- Required: 42
- Optional: 105

<a id="formality-mdd"></a>
## MDD

- All elements: 31
- Narrow (Suppeat): 20
- Required: 13
- Optional: 17

<a id="formality-mdh"></a>
## MDH

- All elements: 47
- Narrow (Suppeat): 25
- Required: 34
- Optional: 12

<a id="formality-mts"></a>
## MTS

- All elements: 41
- Narrow (Suppeat): 16
- Required: 13
- Optional: 27

<a id="formality-noa"></a>
## NOA

- All elements: 219
- Narrow (Suppeat): 58
- Required: 52
- Optional: 166

<a id="formality-nod"></a>
## NOD

- All elements: 138
- Narrow (Suppeat): 36
- Required: 35
- Optional: 102

<a id="formality-pbk"></a>
## PBK

- All elements: 0
- Narrow (Suppeat): 0
- Required: 0
- Optional: 0

<a id="formality-pxa"></a>
## PXA

- All elements: 72
- Narrow (Suppeat): 22
- Required: 29
- Optional: 42

<a id="formality-pxd"></a>
## PXD

- All elements: 67
- Narrow (Suppeat): 23
- Required: 25
- Optional: 41

<a id="formality-sec"></a>
## SEC

- All elements: 73
- Narrow (Suppeat): 40
- Required: 40
- Optional: 32

<a id="formality-shp"></a>
## SHP

- All elements: 124
- Narrow (Suppeat): 9
- Required: 9
- Optional: 114

<a id="formality-sid"></a>
## SID

- All elements: 12
- Narrow (Suppeat): 6
- Required: 3
- Optional: 8

<a id="formality-srv"></a>
## SRV

- All elements: 145
- Narrow (Suppeat): 34
- Required: 25
- Optional: 119

<a id="formality-vid"></a>
## VID

- All elements: 18
- Narrow (Suppeat): 9
- Required: 8
- Optional: 9

<a id="formality-war"></a>
## WAR

- All elements: 23
- Narrow (Suppeat): 10
- Required: 17
- Optional: 5

<a id="formality-was"></a>
## WAS

- All elements: 51
- Narrow (Suppeat): 11
- Required: 26
- Optional: 24
